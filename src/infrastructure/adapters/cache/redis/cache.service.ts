...
import { REDIS_CLIENT } from 'src/common/configurations/cache.config';
import { ICache } from 'src/infrastructure/ports/cache/cache.interface';
...
export class CacheService implements ICache<any> {
  ...
  generateCacheKey(resource: string, request: Request): string {
    const sortedParamsString = this.getSortedParamsString(request);
    ...
    const hashInput =
      sortedParamsString + sortedQueryString + headersString + localeString;
    const hash = this.generateHash(hashInput);
    return `${resource}_${hash}`;
  }

  /**
   * Sorts the route parameters from the request in alphabetical order and returns them as a string.
   *
   * @param request The Express request object.
   * @returns A sorted parameters string.
   */
  private getSortedParamsString(request: Request): string {
    const sortedParams = Object.fromEntries(
      Object.entries(request.params).sort((a, b) => a[0].localeCompare(b[0])),
    );
